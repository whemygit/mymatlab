...
% 形成日线主连
    inpath='H:\EMPIRE\DATASOURCE\rb\SS2\';
    outpath_zld='H:\EMPIRE\DATASOURCE\RB\ZL2\';
    contract='rb';     % 改写使用
    riqi='2015';       % 改写使用
    period='day';  
    ...
    for i=1:26
        if isempty(b(i).data)~=1 %或==0，即b(i)不为空                         ？？？？？？？？
            time1(i)=b(i).data(1,1);                  % 单个合约的起始日期
            time2(i)=b(i).data(end,1);                % 单个合约的截止日期
        else
            time1(i)=0;
            time2(i)=0;
        end
    ...
    time_min=min(time1);
    time_max=max(time2);
    ts=time_max-time_min;                              % 时间跨度，（可能比实际序列长度要大）
    timest=(time_min:time_max)';                       % 从起始日期到截止日期的全日期序列(可能包含周末和节假日,比ts长度要长) ？？？？？？         
    % 建立标准化结构（全时间）
    dayst=struct('data',[]);
    for i=1:24
    dayst(i).data=zeros(ts,9); % 建立24个（ts*9)的零矩阵
    end
    for co=1:24    % co为合约
        for i=1:ts %i为时间
            [j1,j2]=size(b(co).data); % 确定b(co).data的行数！！！
            for j=1:j1   
                if timest(i)==b(co).data(j,1)                  % i和j长度并不一致，i可能比j大
                    dayst(co).data(i,:)=b(co).data(j,:);       % 单个合约的全时间九个指标数据,times，data，code
                end
            end
        end
    end
